...
if (isset($_POST['tambahPelanggan'])) {
    ...
    if ($tambahkat) {
        echo '<script>alert("Berhasil Menambahkan Data");window.location="pelanggan_biasa.php"</script>';
    } else {
...
if (isset($_POST['updatePelanggan'])) {
    ...
    mysqli_query($conn, "UPDATE pelanggan SET
                nama_pelanggan='$nama_pelanggan',telepon_pelanggan='-',alamat_pelanggan='$alamat_pelanggan'
...
<div class="card">
    <div class="card-header">
        <div class="card-tittle"><i class="fa fa-table me-2"></i> Data Pelanggan Non Member
            <div class="float-right">
                <a href="pelanggan.php"><button type="button" class="btn btn-info btn-xs p-2">
                        Pelanggan Member</button></a>
                <button type="button" class="btn btn-primary btn-xs p-2" data-toggle="modal"
                    data-target="#addpelanggan">
    ...
    <div class="card-body">
        <table class="table table-striped table-sm table-bordered dt-responsive nowrap" id="table" width="100%">
            ...
            <tbody>
                <?php
                $no = 1;
                $data_produk = mysqli_query($conn, "SELECT * FROM pelanggan where telepon_pelanggan = '-' order by idpelanggan ASC");
                ...
                <tr>
                    ...
                    <td>
                        <button type="button" class="btn btn-primary btn-xs" data-toggle="modal"
                            data-target="#editP<?php echo $idpelanggan ?>">
                ...
                <!-- modal edit -->
                <div class="modal fade" id="editP<?php echo $idpelanggan ?>" tabindex="-1" role="dialog"
                    aria-labelledby="ModalTittle2" aria-hidden="true">
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <form method="post">
                                <div class="modal-header">
                                    <h5 class="modal-title" id="ModalTittle2"><i class="fa fa-user mr-1 text-muted"></i>
                                        Edit Pelanggan</h5>
...
if (!empty($_GET['hapus'])) {
    ...
    if ($hapus_data) {
        echo '<script>alert("Berhasi Hapus pelanggan");window.location="pelanggan_biasa.php"</script>';
    } else {
